...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/30 20:32:31 by youngski          #+#    #+#             */
/*   Updated: 2022/11/30 20:32:33 by youngski         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

void	ft_strlcpy(char *dest, char *src, size_t n, int flag)
{
	size_t	i;
...
char	*cp_save_to_ret(t_mynode *temp, char *src, ssize_t i)
{
	...
	if (!ret)
		return (0);
	ft_strlcpy(ret, src, k + 1, 1);
	ft_strlcpy(ret + k, temp->content, i + 1, 1);
	ft_strlcpy(temp->content, (temp->content) + i, (temp->size) - i + 1, 0);
	temp->size -= i;
	free(src);
	if (!ret[0])
	{
